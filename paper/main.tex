...
\pdfinfo{
/Title (Uncertainty-Guided Error Repair in Multi-Step Language Model Agents: When to Target, When to Restart)
...
Prior work has largely assumed that targeted repair is preferable when the broken step can be identified \citep{madaan2023selfrefine, pan2024automatically}. Concurrent analyses of automated program repair agents in software engineering reveal analogous challenges: agents overfit to self-generated tests, struggle with error localization, and exhibit complementary strengths across architectures~\citep{ceka2026understanding}. We challenge the targeted-repair assumption through a large-scale empirical study that reveals a more nuanced picture: the optimal strategy depends critically on trajectory length, error locality, and the phenomenon of \emph{context contamination}---where earlier erroneous reasoning corrupts the model's context even when repair begins at the correct step.
...
\begin{enumerate}
  ...
  \item \textbf{Three key empirical findings} across four benchmarks (HotpotQA, FEVER, MuSiQue, 2WikiMultiHopQA) with a Qwen2.5-32B agent, challenging conventional assumptions about repair:
  \begin{itemize}
    \item Full restart outperforms oracle-targeted repair on multi-hop QA (14.2\% vs.\ 10.9\% on HotpotQA; 21.2\% vs.\ 10.0\% on 2WikiMultiHopQA), suggesting context contamination limits surgical repair.
...
\paragraph{Understanding APR Agent Behavior.}
A parallel line of work examines the decision-making processes of automated program repair (APR) agents---systems that use LLMs to fix software bugs in real codebases. \citet{ceka2026understanding} present the first trace-based analysis of five APR agents on SWE-bench Verified, tracing their full pipelines from issue description to patch validation. Their findings echo several themes from our work: (1) error cascading across agent steps, where early localization failures propagate downstream; (2) overfitting to self-generated tests (17--26\% in open-process agents), analogous to context contamination in our setting; and (3) the potential of ensemble approaches, where complementary agent architectures solve disjoint problem subsets. Their study diagnoses \emph{where} APR agents fail but does not propose repair mechanisms; our work provides the complementary prescriptive contribution---uncertainty-guided localization and backtracking---in the domain of multi-step reasoning agents. Notably, their finding that bug localization context improves reproduction test success by 13.5\% aligns with our result that cascade-aware localization rules improve repair effectiveness, suggesting that upstream context is critical for error recovery across both software engineering and reasoning domains.
...
We evaluate on four benchmarks spanning distinct reasoning types:
...
\begin{itemize}
  ...
  \item \textbf{2WikiMultiHopQA} \citep{ho2020constructing}: 2-hop multi-hop QA with bridge and comparison questions over Wikipedia. 500 stratified questions (by type). 12,576 total dev questions.
\end{itemize}
...
We use Qwen2.5-32B-Instruct-AWQ as the agent model, with AWQ 4-bit quantization for A100-80GB inference. This model represents a strong mid-range open-weight LLM, balancing capability with practical compute requirements. For error annotation (ground-truth labels), we use Qwen2.5-72B-Instruct-AWQ as the judge model.
...
% Pre-place full-width figure so it appears at the top of the Results page
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_cross_dataset_bars.png}
\caption{Repair success rates (\%) across four benchmarks and seven strategy categories. 2WikiMultiHopQA shows the highest absolute fix rates; ensemble strategies achieve 2--4$\times$ higher rates at substantially greater compute cost.}
...
Table~\ref{tab:main_results} presents the core comparison across all four benchmarks. Figure~\ref{fig:cross_bars} visualizes the repair success rates across strategies and datasets.

\begin{table}[t]
\centering
\caption{Repair success rates (\%) across four benchmarks. \textbf{Bold} = best non-ensemble strategy per dataset. All results use Qwen2.5-32B agent with 95\% bootstrap CIs.}
\label{tab:main_results}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Strategy} & \textbf{HotpotQA} & \textbf{FEVER} & \textbf{MuSiQue} & \textbf{2Wiki} \\
\midrule
Random step & 8.1 {\tiny(6.0--10.2)} & 1.2 {\tiny(0.6--1.7)} & 3.8 {\tiny(2.8--4.9)} & 19.0 {\tiny(15.9--22.2)} \\
Oracle targeted & 10.9 {\tiny(8.6--13.3)} & 2.5 {\tiny(1.7--3.3)} & 4.9 {\tiny(3.7--6.0)} & 10.0 {\tiny(7.7--12.5)} \\
Oracle + bt2 & 12.4 {\tiny(10.0--14.9)} & \textbf{3.2} {\tiny(2.3--4.0)} & 5.6 {\tiny(4.4--6.8)} & 22.0 {\tiny(18.7--25.3)} \\
Oracle + informed & 11.4 {\tiny(9.0--13.9)} & 2.5 {\tiny(1.8--3.4)} & 4.9 {\tiny(3.8--6.0)} & 12.2 {\tiny(9.5--14.8)} \\
Full restart & \textbf{14.2} {\tiny(11.7--16.8)} & 2.8 {\tiny(2.0--3.7)} & \textbf{5.9} {\tiny(4.6--7.2)} & 21.2 {\tiny(18.1--24.5)} \\
Best unc.+bt2 & 13.2 {\tiny(10.7--15.7)} & 3.4 {\tiny(2.5--4.3)} & 5.6 {\tiny(4.3--6.9)} & \textbf{25.4} {\tiny(22.0--28.9)} \\
\midrule
Ensemble (any) & 36.0 {\tiny(32.5--39.7)} & 9.6 {\tiny(8.1--11.1)} & 16.2 {\tiny(14.3--18.3)} & 46.0 {\tiny(42.0--49.9)} \\
\bottomrule
\end{tabular}
\end{table}
...
On 2WikiMultiHopQA, oracle-targeted repair without backtracking is strikingly weak (10.0\%)---even below random step (19.0\%). However, with two-step backtracking, oracle-targeted repair (22.0\%) slightly exceeds full restart (21.2\%), and the best uncertainty strategy with backtracking achieves 25.4\%. This confirms that backtracking is essential on this dataset, and that uncertainty-guided repair can surpass all baselines when combined with upstream correction.
...
\textbf{Strategy} & \textbf{HotpotQA} & \textbf{FEVER} & \textbf{MuSiQue} & \textbf{2Wiki} \\
\midrule
Oracle targeted & 10.9 & 2.5 & 4.9 & 10.0 \\
Oracle + bt2 & 12.4 (+1.5) & 3.2 (+0.7) & 5.6 (+0.7) & 22.0 (+12.0) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Finding 2: Backtracking consistently helps.} Across all four benchmarks, starting the repair two steps upstream of the identified failure point improves success rates (Table~\ref{tab:backtrack} and Figure~\ref{fig:bt_gain}). The effect is most dramatic on 2WikiMultiHopQA, where backtracking more than doubles the oracle fix rate (10.0\% $\to$ 22.0\%, a +12.0 pp gain). This confirms the error cascade hypothesis: errors propagate forward through reasoning chains, and the true root cause often precedes the step where uncertainty peaks or the judge identifies failure.
...
\begin{figure}[!b]
\centering
\includegraphics[width=\columnwidth]{figures/fig_backtracking_gain.png}
\caption{Left: Oracle, Oracle+bt2, and Full Restart fix rates. Right: Backtracking gain (Oracle+bt2 $-$ Oracle). The +12.0 pp gain on 2WikiMultiHopQA confirms strong error cascade effects.}
\label{fig:bt_gain}
\end{figure}
...
\textbf{Strategy} & \textbf{HotpotQA} & \textbf{FEVER} & \textbf{MuSiQue} & \textbf{2Wiki} \\
\midrule
Random step & 8.1 & 1.2 & 3.8 & 19.0 \\
Best unc.\ strategy & 13.2 & 3.4 & 5.6 & 25.4 \\
\midrule
$\Delta$ & +5.1 & +2.2 & +1.8 & +6.4 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Finding 3: Uncertainty outperforms random.} Across all benchmarks, the best uncertainty-guided strategy substantially outperforms random-step repair (Table~\ref{tab:unc_vs_random}). On 2WikiMultiHopQA, the gap is the largest at 6.4 percentage points (19.0\% $\to$ 25.4\%); on HotpotQA, 5.1 points (8.1\% $\to$ 13.2\%); on FEVER, 2.2 points; on MuSiQue, 1.8 points. This demonstrates that internal model uncertainty signals---despite imperfect calibration---carry actionable information about error locations.

\subsection{Localization Analysis}

Table~\ref{tab:localization} reports localization accuracy across metrics and datasets (visualized in Figure~\ref{fig:loc_heatmap}).

\begin{table}[t]
\centering
\caption{Localization accuracy (argmax top-1 exact match) by uncertainty metric. Self-consistency is the strongest localizer across all datasets. Random baseline shown for reference.}
\label{tab:localization}
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \textbf{HotpotQA} & \textbf{FEVER} & \textbf{MuSiQue} & \textbf{2Wiki} \\
\midrule
Self-consistency & \textbf{0.248} & \textbf{0.223} & \textbf{0.129} & \textbf{0.404} \\
Token entropy (max) & 0.195 & 0.210 & 0.099 & 0.246 \\
Max token prob (max) & 0.177 & 0.212 & 0.099 & 0.232 \\
Perplexity & 0.173 & 0.171 & 0.064 & 0.167 \\
Verbalized confidence & 0.088 & 0.167 & 0.085 & 0.059 \\
\midrule
Random baseline & 0.223 & 0.304 & 0.153 & 0.236 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Self-consistency is the best localizer.} Across all datasets, self-consistency achieves the highest argmax top-1 accuracy, reaching 40.4\% on 2WikiMultiHopQA---the only dataset where it substantially exceeds the random baseline (0.404 vs.\ 0.236). On HotpotQA, it reaches 24.8\%. Notably, on FEVER and MuSiQue, even the best uncertainty metric underperforms the random baseline for exact localization---yet uncertainty-guided \emph{repair} still outperforms random repair, suggesting that approximate localization (within 1--2 steps) is sufficient for effective repair, especially with backtracking.

\begin{figure}[!b]
\centering
\includegraphics[width=\columnwidth]{figures/fig_localization_heatmap.png}
\caption{Localization accuracy (argmax top-1) by uncertainty metric and dataset. Self-consistency achieves the highest accuracy across all benchmarks, reaching 0.404 on 2WikiMultiHopQA.}
\label{fig:loc_heatmap}
\end{figure}

\paragraph{Localization degrades with trajectory complexity.} MuSiQue (2--4 hops, longer trajectories) shows the lowest localization accuracy (12.9\% top-1 for self-consistency vs.\ 40.4\% on 2WikiMultiHopQA), reflecting the difficulty of pinpointing errors in longer reasoning chains where uncertainty is more diffuse. The high localization accuracy on 2WikiMultiHopQA likely contributes to the strong performance of uncertainty-guided repair on this dataset.
...
\midrule
\multicolumn{3}{@{}l}{\emph{2WikiMultiHopQA}} \\
Random step & 170 & 19.0 \\
Best unc.\ + bt2 & 239 & 25.4 \\
Full restart & 246 & 21.2 \\
Ensemble (any) & 11,426 & 46.0 \\
\bottomrule
\end{tabular}
\end{table}

The ensemble strategy---which succeeds if \emph{any} member strategy produces a correct answer---achieves 36.0\% on HotpotQA, 9.6\% on FEVER, 16.2\% on MuSiQue, and 46.0\% on 2WikiMultiHopQA (Table~\ref{tab:cost}). However, this comes at 47--70$\times$ the compute cost of individual strategies. Notably, 2WikiMultiHopQA shows the highest absolute fix rates across all strategies, likely due to the lower failure rate (203/500 vs.\ 226--485/500 on other datasets) indicating that these failures are more ``repairable.'' This highlights a fundamental tradeoff: practitioners must choose between cheap-but-moderate repair (single strategy) and expensive-but-effective repair (ensemble).

\subsection{Informed vs.\ Generic Nudges}

Counter to expectations, error-type-specific informed nudges do not consistently improve over generic nudges. On HotpotQA, oracle-targeted with informed nudge (11.4\%) slightly underperforms the generic variant (10.9\% without nudge, 12.4\% with backtrack). Similarly on FEVER, informed nudges (2.5\%) do not improve over the bt2 variant (3.2\%). On 2WikiMultiHopQA, the same pattern holds: oracle + bt2 + informed (21.0\%) slightly trails oracle + bt2 (22.0\%). We hypothesize that the generic retry hint already triggers sufficient reconsideration, and specific hints may over-constrain the model's exploration.
...
\paragraph{Context contamination.} The hit\_true\_step metric reveals that full restart ``accidentally'' hits the oracle step 31.9\% of the time on HotpotQA (42.8\% on MuSiQue, 3.0\% on 2WikiMultiHopQA), compared to 100\% for oracle-targeted. Despite this disadvantage, full restart still wins on HotpotQA and MuSiQue, confirming that the benefit of a clean context outweighs the cost of not targeting the specific broken step.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig_traj_length_vs_repair.png}
\caption{Trajectory length vs.\ repair effectiveness. Shorter trajectories (FEVER, 2WikiMHQA) favor targeted repair; longer trajectories (HotpotQA, MuSiQue) favor full restart, consistent with the context contamination hypothesis.}
\label{fig:traj_repair}
\end{figure}

\paragraph{Trajectory length as moderator.} FEVER trajectories average 3.9 tool calls vs.\ 4.7 for HotpotQA, 4.0 for 2WikiMultiHopQA, and 6.6 for MuSiQue. Shorter trajectories have less accumulated context contamination, explaining why targeted repair succeeds on FEVER but not on the longer multi-hop QA tasks. Interestingly, 2WikiMultiHopQA has similar trajectory length to FEVER (4.0 tool calls) and shows a similar pattern: oracle + bt2 (22.0\%) slightly exceeds full restart (21.2\%), and the best uncertainty strategy (25.4\%) surpasses both. This further supports the heuristic: \emph{use targeted repair with backtracking for short trajectories and full restart for long ones}.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/fig_pareto_all_datasets.png}
\caption{Cost-accuracy Pareto fronts for all four datasets. Each grey dot is an individual repair strategy; labeled points show baselines. On 2WikiMultiHopQA, many uncertainty strategies dominate the Pareto front above Full Restart.}
\label{fig:pareto}
\end{figure*}
...
\paragraph{Cross-Domain Parallels with Program Repair Agents.}
Our findings resonate with recent analyses of automated program repair (APR) agents in software engineering~\citep{ceka2026understanding}. First, the structural parallel between their \emph{workflow vs.\ open-process} agent dichotomy and our \emph{targeted repair vs.\ full restart} tradeoff is striking: workflow agents produce concise, localized patches ($\sim$22 lines, single-file edits) much like targeted repair preserves a correct prefix, while open-process agents generate broad rewrites spanning multiple files and hundreds of lines, akin to full restart regenerating the entire trajectory. Their finding that workflow agents excel on simple tasks and open-process agents on complex ones directly mirrors our trajectory-length moderator---targeted repair for short chains, full restart for long ones---suggesting this tradeoff may be a domain-general property of sequential agent recovery.
Second, their ``shift-left'' recommendation---moving test generation earlier in the pipeline to prevent error propagation---is the temporal analogue of our cascade-aware localization rules, which intervene upstream of the uncertainty peak rather than at the symptom. Both address the same root cause: errors compound forward through sequential decision-making, and earlier intervention is more effective than downstream correction. Third, their inference-time scaling analysis reveals that low temperature ($t{=}0.2$) with more samples outperforms high-temperature diversity for test generation, echoing our finding that informed (more specific) nudges do not consistently beat generic ones---in both cases, over-constraining the search space reduces rather than improves recovery. Finally, their observation that APR agents operate with primitive tooling (bash scripts, no debuggers or program analyzers) underscores the value of internal uncertainty signals: our metrics provide a form of structured self-introspection that current agents---across both reasoning and code repair domains---largely lack.

\paragraph{Limitations.} Our main experiments use a single agent model (Qwen2.5-32B) across four benchmarks. Future work will extend the evaluation to eight models across three capacity tiers (7--72B) to test whether the observed patterns---especially the dominance of full restart on long trajectories---generalize across model families and scales. Additionally, we use offline document collections rather than live retrieval, which limits generalization to open-ended web agents. The 72B judge, while validated against human labels, may introduce systematic biases in error annotation. We evaluate only ReAct-style agents; other architectures (\eg, tree search, iterative refinement) may exhibit different repair dynamics.
...
We presented a systematic study of repair strategies for multi-step LLM agents, evaluating 33 strategies across four benchmarks with a 32B instruction-tuned agent. Our key finding is that the optimal repair strategy depends on trajectory length: full restart is preferable for long multi-hop reasoning chains due to context contamination, while targeted repair with backtracking succeeds on shorter trajectories. On 2WikiMultiHopQA, where backtracking more than doubles the oracle fix rate (10.0\% $\to$ 22.0\%), uncertainty-guided strategies achieve the highest individual fix rates (25.4\%), surpassing all baselines. Uncertainty-guided localization consistently outperforms random baselines across all four datasets, validating internal model signals for error diagnosis. Our cascade-aware localization rules, which account for forward error propagation, improve upon standard uncertainty-based approaches. These findings provide practical guidance for deploying self-repairing LLM agents and highlight the under-explored role of context contamination in sequential reasoning.
