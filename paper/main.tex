...
Table~\ref{tab:main_results} presents the core comparison across all four benchmarks.
...
\paragraph{Finding 2: Backtracking consistently helps.} Across all four benchmarks, starting the repair two steps upstream of the identified failure point improves success rates (Table~\ref{tab:backtrack}). The effect is most dramatic on 2WikiMultiHopQA, where backtracking more than doubles the oracle fix rate (10.0\% $\to$ 22.0\%, a +12.0 pp gain). This confirms the error cascade hypothesis: errors propagate forward through reasoning chains, and the true root cause often precedes the step where uncertainty peaks or the judge identifies failure.
